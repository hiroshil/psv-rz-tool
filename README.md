...
virtual range `0x81000000..0x81100000` against the known-good VWF-patcher output,
then patches only SC allocation, SC metadata and the runtime script buffer size.
Passing stock EBOOT in that VWF path is rejected because diagnostic testing
isolated it as the runtime hang path. The input ELF is never overwritten.
...
2. pass that VWF-patched ELF to `rz-tool build --eboot-in/--eboot-out` so
   `rz-tool` can patch only SC allocation, SC metadata and script buffer sizing.
...
```text
rz-tool extract-alloc patched_eboot.bin.elf sc-allocation.json
```
