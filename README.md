...
The stable project root is `rz-project.json` with `schema_version: 1`.
Rebuild-authoritative image, script, font and charset documents also use
`document_version: 1`. A build directory is expected to be generated by this
extractor so that machine-managed source buffers, fingerprints and metadata are
available to the corresponding rebuild path.

## Supported resources

| Resource | Editable representation | Status |
|---|---|---|
| `sc.cpk` | `NNNNN.script.json` + `NNNNN.script-meta.json`, shared `charset.json` | relocatable Unicode script editing; optional companion ELF patch when allocations grow |
...
Generated asset files are written at the project root. The extractor uses the
CPK entry order as the stable five-digit filename prefix. Paths in the manifest
must remain relative to the project and may not escape the project directory.
...
An editable `sc.cpk` project creates exactly two JSON documents per entry:
...
`NNNNN.script.json` is the user-editable, source-equivalent IR: labels, Unicode
dialogue nodes and exact raw `u16` nodes for commands or data whose full opcode
...
`charset.json` is generated once per editable `sc.cpk` project. It maps Unicode
codepoints to the existing glyph IDs accepted by the engine and is also the
default charset map used by `rz-tool build` unless `--charset-map` supplies a
different document.
...
Extraction creates exactly two JSON documents per entry:
...
The editable document contains labels, Unicode dialogue and exact raw `u16`
nodes for unclassified commands/data. The meta document contains allocation,
voice header, footer, secondary relocation records and engine annotations.
There are no script skeleton binaries.
